...
@section('content')
    ...
    <div class="py-10 px-20 bg-grey-100">
            <h1 class="text-4xl font-bold">Related product</h1>
            <div class="grid grid-cols-4 gap-5 mt-5">
                ...
                <a href="{{route('viewproduct',$product->id)}}" class="shadow-md p-2 border">
                    ...
                    <div class="mt-2">
                        <h1 class="text-xl font-bold">{{$product->name}}</h1>
                        @if($product->discounted_price>0)
                        <p class="text-md font-bold">Rs.
                        {{$product->discounted_price}}<span class="line-through font-normal text-sm">Rs. {{$product->price}}</span></p>
                        @else
